...
class CreditControllerTest extends SpringBootMvcBaseTest {
    ...
    void getInfoUnauthorizedNoToken() {
        ...
        // when
        var rsDto = performGetWithDto("/credit-service/api/v1/credit/" + creditId, ErrorRsDto.class, status().isUnauthorized(), token);
    ...
    void getInfoUnauthorizedInvalidToken() {
        ...
        // when
        var rsDto = performGetWithDto("/credit-service/api/v1/info/" + creditId, ErrorRsDto.class, status().isUnauthorized(), token);
    ...
    void getInfoNotFound() {
        ...
        // when
        var rsDto = performGetWithDto("/credit-service/api/v1/info/" + creditIdNotFound, ErrorRsDto.class, status().isNotFound(), token);
    ...
    void getInfoByOwner() {
        ...
        var createdRsDto = performPostWithDto("/credit-service/api/v1/create", createRqDto, CreditCreateRsDto.class, status().isOk(), token);

        // when
        var infoRsDto = performGetWithDto("/credit-service/api/v1/info/" + createdRsDto.id(), CreditInfoRsDto.class, status().isOk(), token);
    ...
    void getInfoByCreditManager() {
        ...
        var createRqDto = buildCreditCreateRqDto();
        var createdRsDto = performPostWithDto("/credit-service/api/v1/create", createRqDto, CreditCreateRsDto.class, status().isOk(), creditOwnerToken);
        var creditManagerToken = jwtGenerator.generate(Set.of(ROLE_CREDIT_MANAGER));

        // when
        var infoRsDto = performGetWithDto("/credit-service/api/v1/info/" + createdRsDto.id(), CreditInfoRsDto.class, status().isOk(), creditManagerToken);
    ...
    void getInfoForbiddenForNoManager() {
        ...
        // when
        var rsDto = performGetWithDto("/credit-service/api/v1/info/" + createdRsDto.id(), ErrorRsDto.class, status().isForbidden(), noManagerToken);
